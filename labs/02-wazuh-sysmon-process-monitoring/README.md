...
> Adding the Sysmon channel to `ossec.conf` is a collection configuration, not a detection rule.
...
- **`providerName`:** Event source, in this case Microsoft-Windows-Sysmon.
...
![Sysmon Event ID 1 showing calc.exe process creation](screenshots/01-sysmon-process-create-calc-commandline.png.JPG)

### Parent Process Analysis

The event shows that `calc.exe` was launched by `powershell.exe`, which is useful for process tree analysis.

![Parent process analysis showing powershell.exe launching calc.exe](screenshots/02-sysmon-parent-process-powershell.png.JPG)

### Sysmon Event ID 1 Details

The event was collected from the `Microsoft-Windows-Sysmon/Operational` channel.

![Sysmon Event ID 1 details from the Operational channel](screenshots/03-sysmon-event-id-1-details.png.JPG)
...
![Wazuh Rule 92066 and MITRE ATT&CK T1059.001 mapping](screenshots/04-wazuh-rule-92066-mitre-t1059-001.png.JPG)
...
To collect Sysmon events, the agent must be configured to read:
