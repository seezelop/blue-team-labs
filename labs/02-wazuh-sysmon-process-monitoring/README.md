...
![Sysmon Event ID 1 showing calc.exe process creation](screenshots/sysmon-event-id-1-calc.png)

### Parent Process Analysis

The event shows that `calc.exe` was launched by `powershell.exe`, which is useful for process tree analysis.

![Parent process analysis showing powershell.exe launching calc.exe](screenshots/parent-process-powershell-calc.png)

### Sysmon Event ID 1 Details

The event was collected from the `Microsoft-Windows-Sysmon/Operational` channel.

![Sysmon Event ID 1 details from the Operational channel](screenshots/sysmon-event-id-1-details.png)
...
![Wazuh Rule 92066 and MITRE ATT&CK T1059.001 mapping](screenshots/wazuh-rule-92066-mitre-t1059-001.png)
